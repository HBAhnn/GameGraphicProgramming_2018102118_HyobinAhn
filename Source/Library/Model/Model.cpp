...
namespace library
{
    ...
    HRESULT Model::Initialize(_In_ ID3D11Device* pDevice, _In_ ID3D11DeviceContext* pImmediateContext)
    {
        ...
        hr = pDevice->CreateBuffer(&bd, &animData, m_animationBuffer.GetAddressOf());
        ...
        hr = pDevice->CreateBuffer(&bd2, nullptr, m_skinningConstantBuffer.GetAddressOf());
    ...
    void Model::Update(_In_ FLOAT deltaTime)
    {
        ...
        if (m_pScene->HasAnimations())
        {
            ...
            if (m_pScene->mRootNode)
            {
                ...
                for (int i = 0; i < m_aBoneInfo.size(); i++)
                {
        ...
        _In_ const std::filesystem::path& filePath)
    {
        ...
        //리사이즈도 countVerticesAndIndices에서 진행
        ...
        for(int i = 0 ; i < GetNumVertices() ; i++)
        {
            for (int j = 0; j < 16; j++)
            {
                m_aAnimationData[i].aBoneIndices = XMUINT4(m_aBoneData[i].aBoneIds[0], m_aBoneData[i].aBoneIds[1], m_aBoneData[i].aBoneIds[2], m_aBoneData[i].aBoneIds[3]);
                m_aAnimationData[i].aBoneWeights = XMFLOAT4(m_aBoneData[i].aWeights[0], m_aBoneData[i].aWeights[1], m_aBoneData[i].aWeights[2], m_aBoneData[i].aWeights[3]);
            }
        }
    ...
    void Model::interpolateRotation(_Inout_ XMVECTOR& outQuaternion, _In_ FLOAT animationTimeTicks, _In_ const aiNodeAnim* pNodeAnim)
    {
        ...
        outQuaternion = XMVectorSet(out.x, out.y, out.z, out.w);
    }
    ...
    void Model::readNodeHierarchy(_In_ FLOAT animationTimeTicks, _In_ const aiNode* pNode, _In_ const XMMATRIX& parentTransform)
    {
        ...
        if (pNodeAnim) { 
            XMFLOAT3 scalingMatrix = XMFLOAT3();
            interpolateScaling(scalingMatrix, animationTimeTicks, pNodeAnim);

            XMVECTOR rotationMatrix = XMVECTOR();
            interpolateRotation(rotationMatrix, animationTimeTicks, pNodeAnim);

            XMFLOAT3 translationMatrix = XMFLOAT3();
            interpolatePosition(translationMatrix, animationTimeTicks, pNodeAnim);

            XMMATRIX scaling = XMMatrixScaling(scalingMatrix.x, scalingMatrix.y, scalingMatrix.z);
            XMMATRIX rotation = XMMatrixRotationQuaternion(rotationMatrix);
            XMMATRIX transposition = XMMatrixTranslation(translationMatrix.x, translationMatrix.y, translationMatrix.z);

            nodeTransformation = scaling * rotation * transposition;
        }
        XMMATRIX globalTransformation = nodeTransformation * parentTransform;

        if (m_boneNameToIndexMap.contains(pNode->mName.C_Str()))
        {
            UINT boneidx = m_boneNameToIndexMap.at(pNode->mName.C_Str());
